"""
File Analysis Agent — sends STRIPPED code to LLM.
"""
...
def _detect_interactions(
    file_path: str, content: str, refs: list[str], language: str,
) -> list[FileInteraction]:
    interactions = []
    for ref in refs:
        ref_bn = ref.split("/")[-1]
        if "executeScript" in content and ref_bn in content:
            itype, desc = "injects", f"{file_path} injects {ref} via chrome.scripting.executeScript"
        elif file_path.endswith("manifest.json"):
            itype, desc = "configures", f"manifest.json declares {ref}"
        elif "require(" in content or "import " in content:
            itype, desc = "imports", f"{file_path} imports {ref}"
        else:
            itype, desc = "references", f"{file_path} references {ref}"
        interactions.append(FileInteraction(
            source_file=file_path, target_file=ref,
            interaction_type=itype, description=desc,
        ))
    return interactions
...
async def analyze_file(
    ...
    # Static analysis on FULL content (needs string literals, imports, etc.)
    static_refs = _extract_file_references(content, all_project_files)
    static_interactions = _detect_interactions(file_path, content, static_refs, metadata.language)

    # Strip code for LLM (removes comments, blanks, docstrings)
    stripped_content, strip_stats = strip_for_llm(content, file_path)
    logger.info(
        "📦 %s: %d→%d lines (-%s%% tokens saved)",
        file_path, strip_stats["original_lines"],
        strip_stats["stripped_lines"], strip_stats["saved_percent"],
    )
...
{{"file_path": "{file_path}", "summary": "what this file does", "functions": [{{"name": "func_name", "description": "what it does", "calls": ["functions it calls"], "imports_used": []}}], "classes": [{{"name": "class_name", "methods": ["method1"]}}], "exports": [], "external_dependencies": ["third party packages"], "internal_file_references": ["other project files used"], "file_interactions": [{{"source_file": "{file_path}", "target_file": "other.js", "interaction_type": "imports", "description": "how"}}]}}"""
    ...
    try:
        ...
        logger.info("✓ %s (funcs=%d, refs=%d, interactions=%d)",
                     file_path, len(result.functions),
                     len(result.internal_file_references), len(result.file_interactions))
        return result

    except Exception as e:
        logger.error("✗ %s: %s — tree-sitter fallback", file_path, e)
        return _fallback(file_path, content, parsed, static_refs, static_interactions)


def _fallback(file_path, content, parsed, refs, interactions) -> FileAnalysisResult:
    functions = [
        FunctionAnalysis(name=f.name, description=f"Function at lines {f.start_line}-{f.end_line}")
        for f in parsed.functions
    ]
    classes = [ClassAnalysis(name=c.name, methods=c.methods) for c in parsed.classes]
